...
interface SystemData {
  ...
  binding: { lockedOn: boolean; endeavor: string | null; angelsUrl: string | null; channel: string | null; tokenValid: boolean }
...
export default function NodeDashboard() {
  ...
  // The lock-on state — which Endeavor this Merlin is bound to. SINGLE SOURCE OF
  // TRUTH = the server's node binding (/api/system → binding), because the
  // autonomous heartbeat runs off it regardless of any browser session. The client
  // session (useConnection) is only an ENRICHMENT for the signed-in user's email.
  // Everything on this screen now agrees off `lockedOn`. (Ponytail.)
  const { active } = useConnection()
  const lockedOn = Boolean(sys?.binding?.lockedOn || active)
  const boundLabel = sys?.binding?.endeavor || active?.domain || active?.slug || null
  ...
  return (
    <motion.div variants={container} initial="hidden" animate="show" className="space-y-5">
        ...
        >
          ...
          <div className="relative min-w-0 flex-1">
            ...
            <div className="truncate text-lg font-mono font-semibold leading-tight">
              {lockedOn ? boundLabel : 'Lock onto an Endeavor'}
            </div>
            <div className="truncate text-[11px] text-muted-foreground">
              {lockedOn
                ? active?.user?.email
                  ? `Signed in as ${active.user.email} · this node is contributing to the federation`
                  : 'This node is locked on and contributing to the federation.'
      ...
      <motion.div variants={item} className="flex items-end justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            {lockedOn
              ? sys?.angels.online
                ? `Locked onto ${boundLabel} · ${sys.angels.responseMs}ms`
                : `Locked onto ${boundLabel} · reconnecting…`
              : sys?.angels.online
                ? 'Federation reachable · not locked on yet'
                : 'Local node standing by. Ad Astra.'}
          </p>
        </div>
      ...
      <motion.div variants={container} className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
        <StatCard label="Federation" value={lockedOn ? 'LINKED' : sys ? 'LOCAL' : '···'}
          sub={lockedOn ? (boundLabel || 'locked on') : sys?.angels.online ? 'reachable · not locked on' : 'connect via initial node'}
          icon={Radio} accent={lockedOn ? '#22cc88' : '#f5a623'} href="/connect" />
